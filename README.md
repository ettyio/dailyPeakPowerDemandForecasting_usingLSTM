...
기상청 기상 데이터와 전력거래소 전력 소비 데이터를 결합하여 2025년 일일 최대 전력 수요를 예측하는 LSTM 프로젝트입니다. 
서로 다른 출처(이기종)의 시계열 데이터를 통합하고, 이상치에 강한 모델을 구축하여 **MAPE 4.49%**의 성능을 달성했습니다.
...
데이터의 수집부터 모델 학습, 결과 예측까지 이어지는 End-to-End 파이프라인 구축
...
API 타임아웃 예외 처리: 대용량 기상 데이터 요청 시 발생하는 연결 끊김 문제를 해결하기 위한 재시도 로직 구현.
이기종 데이터 통합: 포맷이 다른 CSV 파일과 API JSON 응답 데이터를 파싱하여 Timestamp 기준으로 정밀 매핑.

2. 피처 엔지니어링 (utils.py)
단순 기온 정보의 한계를 극복하기 위해 파생변수를 생성하여 예측력 향상.
이동평균(Rolling): 과거 3일간의 기온 추세를 반영하는 T_avg_Roll3 변수 생성.
체감 지표: 습구온도(WetBulb), 불쾌지수(Discomfort Index) 등 전력 소비에 영향을 주는 기상 요소 수식 구현.

3. 모델 최적화 및 안정성 (model.py, train.py)
Huber Loss 적용: 전력 데이터 특성상 발생하는 돌발적인 이상치에 모델이 과도하게 반응하지 않도록, MSE 대신 Huber Loss를 손실 함수로 채택하여 학습 안정성 확보.
Sliding Window: 시계열 데이터의 연속성을 학습하기 위한 시퀀스 데이터셋 구성.
...
🚀 설치 및 실행 방법
1. 리포지토리 클론
git clone https://github.com/ettyio/dailyPeakPowerDemandForecasting_usingLSTM.git
...
3. 환경 변수 설정
프로젝트 루트 경로에 .env 파일을 생성하고 아래 내용을 입력
